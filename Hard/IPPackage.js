 ...
 */
function fixPackage(line){
    var ar = line.split(" ").filter(function (value, index) { return index < 22; });
    
    var ip1=ar.shift();
    var ip2=ar.shift();
    
    var checksum = ar[10] + ar[11];
    ar[10] = "00";
    ar[11] = "00";

    var oldCheckSum = createCheckSum(ar);
    if (oldCheckSum != checksum) { throw "Invalid header"; }

    // replace IP (12-15),(16-19)
    IpToHex(ip1).forEach(function (v, i) { ar[12 + i] = v; });
    IpToHex(ip2).forEach(function (v, i) { ar[16 + i] = v; });

    // calculate new checksum
    var newCheckSum = createCheckSum(ar);

    // replace checksum
    ar[10] = newCheckSum.substr(0,2);
    ar[11] = newCheckSum.substr(2, 2);
...
function IpToHex(ip) {
    return ip.split(".")
        ...
        .map(intToHex)
        .map(function (v) { return pad(v, 2, "0"); });
}

function create16BitValues(acc, value, index) {
    if (index % 2 == 0) { acc.push(value); }
    else { acc[acc.length - 1] = acc[acc.length - 1] + value; }
    return acc;
}

function hexToInt(hexString) { return parseInt(hexString, 16); }
function intToBin(intValue) { return intValue.toString(2); }
function binToInt(binValue) { return parseInt(binValue, 2); }
function intToHex(value) { return value.toString(16); }
...
function flipBits(binValue) {
    return binValue
        .split("")
        .map(function (value) { return value == "1" ? "0" : "1"; })
        .join("");
}

function pad(value, size, char) {
    while (value.length < size) { value = char + value; }
    return value;
}

function createCheckSum(hexArray) {

    var sum = hexArray.reduce(create16BitValues, [])
        .map(hexToInt)
        .reduce(addWithCarry);

    var paddedValue = pad(intToBin(sum), 16, "0");
    var flippedValue = flipBits(paddedValue);

    return pad(intToHex(binToInt(flippedValue)), 4, "0");
}




describe("createCheckSum", function () {
    it("test1", function () {
        expect(createCheckSum("45 00 00 3c 1c 46 40 00 40 06 00 00 ac 10 0a 63 ac 10 0a 0c".split(" "))).toEqual("b1e6");
    });
});
...
describe("fixPackage", function () {
    ...
    var s1Expected = "45 04 05 dc b7 3a 40 00 2e 06 11 cd be a8 00 60 be a8 00 60";
    it("s1", function () {
        expect(fixPackage(s1)).toEqual(s1Expected);
    });
});
